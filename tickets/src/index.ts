...
import { OrderCanceledConsumer } from './events/consumers/order-canceled-comsumer'
...
const start = async () => {
    ...
    try {
        ...
        await new OrderCanceledConsumer(rabbitWrapper.consummerConnection).listen()

        await mongoose.connect(process.env.MONGO_URI) // change to other db on product 😉
        console.log('[MONGODB] Connected')

        server = app.listen(3000, () => {
            console.log('Tickets service run on port 3000 😎')
        })
    } catch (error: any) {
